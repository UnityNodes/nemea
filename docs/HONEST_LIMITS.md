...
- **Tokenised real-world assets are watched against an average, not a truth.** For a tokenised stock or commodity, Nemea compares the wrapper you hold to `average_tokenized_price` across every tokenised version of that asset. Three limits follow. The average is refreshed roughly every 8 hours on a free plan, so a drift smaller than a few per cent is not reliable and an anchor older than 12 hours is ignored rather than used (`MAX_RWA_AGE_MS`, `packages/alerts/test/rwa.test.ts`). Only the top 300 assets by CoinMarketCap rank are watched, not all 7,942. And wrappers denominate different units of the same asset with nothing in the API to say so: `GRAMS` prices silver per gram against a per-ounce average and reads 93% low. Anything past 25% is therefore recorded as a unit mismatch instead of sent, which means a genuine dislocation larger than 25% is also not sent. `RWA_UNIT_MISMATCH_PCT` is the line, and the trade is deliberate: a wrong alert every poll forever costs more than a missed extreme.
- **Tokenised asset data is only fetched while someone holds one.** A portfolio with no wrapper costs nothing and the lane reads "only when held" on `/status`, so the stablecoin peg check keeps its full cadence.
