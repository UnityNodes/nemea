...
| Wallet import matched against real CoinMarketCap | `pnpm gate:b` | **passed 2026-09-22** on a public wallet: 79 tokens matched by chain and contract address across Ethereum, Base and Arbitrum, native ETH on all three, unmatched tokens skipped with a reason, evidence in `docs/evidence/gate-b-wallet.json`. The count moves between runs because the sample wallet trades. The gate reads public Blockscout, which sometimes times out on a wallet with thousands of balances; one failed run in three is normal and the gate reports it rather than passing quietly |
